...
class TerraformApplyPlugin(wrapper: PluginWrapper): Plugin(wrapper) {

    override fun start() {
        System.out.println("TerraformApplyPlugin.start()")
    }

    override fun stop() {
        System.out.println("TerraformApplyPlugin.stop()")
    }
}

@Extension
class TerraformApplyPreconfiguredStage(val pluginSdks: PluginSdks, val configuration: PluginConfig) : PreconfiguredJobConfigurationProvider {
    override fun getJobConfigurations(): List<KubernetesPreconfiguredJobProperties> {
        val jobProperties = pluginSdks.yamlResourceLoader().loadResource("io/opsmx/plugin/preconfigured/TSApplyJob.yaml", KubernetesPreconfiguredJobProperties::class.java)
