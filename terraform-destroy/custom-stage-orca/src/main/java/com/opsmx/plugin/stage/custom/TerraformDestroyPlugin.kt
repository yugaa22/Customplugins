...
class TerraformDestroyPlugin(wrapper: PluginWrapper): Plugin(wrapper) {

    override fun start() {
        System.out.println("TerraformDestroyPlugin.start()")
    }

    override fun stop() {
        System.out.println("TerraformDestroyPlugin.stop()")
    }
}

@Extension
class TerraformDestroyPreconfiguredStage(val pluginSdks: PluginSdks, val configuration: PluginConfig) : PreconfiguredJobConfigurationProvider {
    override fun getJobConfigurations(): List<KubernetesPreconfiguredJobProperties> {
        val jobProperties = pluginSdks.yamlResourceLoader().loadResource("io/opsmx/plugin/preconfigured/TSDestroyJob.yaml", KubernetesPreconfiguredJobProperties::class.java)
